...
//----------------------- START MODIFICATIONS SERHANI 11/12/2013 -----------
...
//----------------------- END MODIFICATIONS SERHANI 11/12/2013 -------------
...
{
  //NS_LOG_FUNCTION ("TypeId PacketHeader::GetTypeId (void)");
...
{
  //NS_LOG_FUNCTION ("TypeId MessageHeader::GetTypeId (void)");
...
MessageHeader::Hello::GetSerializedSize (void) const
{
  ...
  for (std::vector<LinkMessage>::const_iterator iter = this->linkMessages.begin ();
       iter != this->linkMessages.end (); iter++)
    {
      ...
      //size += 4;                              //-------- COMMENTED BY SERHANI 12/12/2013 --------------
      size += 6;                                //-------- MODIFICATION BY SERHANI 12/12/2013 -----------
...
MessageHeader::Hello::Serialize (Buffer::Iterator start) const
{
  ...
  for (std::vector<LinkMessage>::const_iterator iter = this->linkMessages.begin ();
       iter != this->linkMessages.end (); iter++)
    {
      ...
      i.WriteHtonU16 (6 + lm.neighborInterfaceAddresses.size () * IPV4_ADDRESS_SIZE); //--- Modified from 4 To 6 By SERHANI ----
    
      i.WriteU8 (lm.RLQ);   //------- MODIFICATION BY SERHANI 12/12/2013 ---------
      i.WriteU8 (lm.ETX);   //------- MODIFICATION BY SERHANI 12/12/2013 ---------
  ...
  while (helloSizeLeft)
    {
      ...
      lm.RLQ = i.ReadU8 ();                 //------- MODIFICATION BY SERHANI 12/12/2013 ---------
      lm.ETX = i.ReadU8 ();                 //------- MODIFICATION BY SERHANI 12/12/2013 ---------
        
      NS_ASSERT ((lmSize - 6) % IPV4_ADDRESS_SIZE == 0); // Modifyed from 4 to 6 By SERHANI
      for (int n = (lmSize - 6) / IPV4_ADDRESS_SIZE; n; --n) // Modifyed from 4 to 6 By SERHANI
        {
...
MessageHeader::Tc::GetSerializedSize (void) const
{
  ...
  //return 4 + this->neighborAddresses.size () * IPV4_ADDRESS_SIZE;         //---- COMMENTED BY SERHANI 12/12/2013 ---------
    return 4 + this->neighborAddresses.size () * (IPV4_ADDRESS_SIZE + 1);   //---- MODIFIED  BY SERHANI 12/12/2013 ---------
}
...
MessageHeader::Tc::Serialize (Buffer::Iterator start) const
{
  ...
  //---- MODIFIED BY SERHANI 12/12/2013 -------
...
MessageHeader::Tc::Deserialize (Buffer::Iterator start, uint32_t messageSize)
{
  ...
  NS_ASSERT ((messageSize - 4) % (IPV4_ADDRESS_SIZE + 1) == 0);         //-------- ADDED +1 by SERHANI 12/12/2013 -------
  int numAddresses = (messageSize - 4) / (IPV4_ADDRESS_SIZE + 1);       //-------- ADDED +1 by SERHANI 12/12/2013 -------
  this->neighborAddresses.clear ();
  this->ETX.clear ();
  for (int n = 0; n < numAddresses; ++n)
  {
    this->neighborAddresses.push_back (Ipv4Address (i.ReadNtohU32 ()));
    this->ETX.push_back (i.ReadU8 ());                                  //------ ADDED BY SERHANI 12/12/2013 -------
  }
