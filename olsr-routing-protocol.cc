...
#define OLSR_MAXIMUM_METRIC 65535
#define OLSR_DEFAULT_METRIC 4096
#define OLSR_UNDEFINED_LQ 0
#define OLSR_ETX_SEQNO_RESTART_DETECTION 256

#define OLSR_ETX_UPDATE_TIME   Time (7 * OLSR_REFRESH_INTERVAL)
...
{
  //NS_LOG_UNCOND ("TypeId RoutingProtocol::GetTypeId (void)");
...
{
  //NS_LOG_UNCOND(Simulator::Now ().GetSeconds () << " PrintRoutingTable");
  //NS_LOG_FUNCTION (this);
    
  std::ostream* os = stream->GetStream ();
  *os << "\n\n-------------------------------------- Routing Table [" << m_mainAddress << "] at " << Simulator::Now ().GetSeconds () << " s --------------------------------------------\n";
...
void RoutingProtocol::DoInitialize ()
{
  ...
  //NS_LOG_DEBUG ("Starting OLSR on node (m_mainAddress => " << m_mainAddress << ")");
...
RoutingProtocol::RecvOlsr (Ptr<Socket> socket)
{
  ...
  //NS_LOG_UNCOND(Simulator::Now ().GetSeconds () << " OLSR Node [" << m_mainAddress << "] Received Packet from [" << inetSourceAddr.GetIpv4 () << "]");
...
{
  //NS_LOG_UNCOND ("void CoverTwoHopNeighbors (Ipv4Address neighborMainAddr, TwoHopNeighborSet & N2)");
...
RoutingProtocol::RoutingTableComputation ()
{
  ...
  //NS_LOG_DEBUG (Simulator::Now ().GetSeconds () << " s: Node " << m_mainAddress << ": RoutingTableComputation begin...");
      ...
      if (nb_tuple.status == NeighborTuple::STATUS_SYM)
        {
              ...
              if ((GetMainAddress (link_tuple.neighborIfaceAddr) == nb_tuple.neighborMainAddr)
                  && link_tuple.time >= Simulator::Now ())
                {
                  ...
                  AddEntry (link_tuple.neighborIfaceAddr,
                            ...
                            1,
                            link_tuple.ETX,                                //--------------- MODIFIED BY SERHANI ------------------
                            false);                                        //--------------- MODIFIED BY SERHANI ------------------
          ...
          if (!nb_main_addr && lt != NULL)
            {
              //NS_LOG_LOGIC ("no R_dest_addr is equal to the main address of the neighbor => adding additional routing entry");
              AddEntry (nb_tuple.neighborMainAddr,
                        lt->neighborIfaceAddr,
                        lt->localIfaceAddr,
                        1,
                        lt->ETX,                                           //------------- MODIFIED BY SERHANI  -------------------
                        false);
            }
        }//End if
    }//end for 2.
      ...
      if (foundEntry)
        {
          ...
          AddEntry (nb2hop_tuple.twoHopNeighborAddr,
                    ...
                    entry.interface,
                    2,
                    entry.metric + nb2hop_tuple.ETX,            //------ MODIFIED BY SERHANI ---------
                    false);
        }
      else
        {
  ...
  for (uint32_t h = 2;; h++)
    {
        //------------------------------------------------------- START ADDED BY SERHANI ---------------------
        
        bool isUnused = false;
        
        //NS_LOG_UNCOND("-------------------------------------------------------------------------------------------- [" << m_mainAddress << "] h [" << h << "] ------");
        ...
        for (std::vector<RoutingTableEntry>::iterator rte = rtEntries.begin (); rte != rtEntries.end (); rte++)
        {
            ...
            //NS_LOG_UNCOND("in 1st loop BEFORE destAddr [" << rte->destAddr << "] nextAddr [" << rte->nextAddr << "] used [" << rte->used << "]");
        ...
        if (isUnused)
        {
            //NS_LOG_UNCOND("After 2nd loop destAddr [" << current_entry.destAddr << "] nextAddr [" << current_entry.nextAddr <<  "] used [" << current_entry.used << "]");
            ...
            for (TopologySet::const_iterator it = topology.begin (); it != topology.end (); it++)
            {
                const TopologyTuple &topology_tuple = *it;
                   ...
                   {
                       //NS_LOG_UNCOND("in 2nd loop: TopologyTuple " << topology_tuple);
                       ...
                       if (!have_destAddrEntry)
                       {
                           ...
                           {
                               //NS_LOG_UNCOND("--------- TopologyTuple ----------" << topology_tuple);
                               //NS_LOG_UNCOND("-----!!!! Add New Entry !!!!-----------");
                       ...
                       else if (new_metric < destAddrEntry.metric)
                       {
                           if (topology_tuple.destAddr != m_mainAddress)
                           {
                               ...
                               //NS_LOG_UNCOND("----- NewDistance = [" << new_distance << "] NewMetric [" << new_metric << "] ------------");
        ...
        if (!isUnused)
        {
            //NS_LOG_UNCOND("break!! destAddr [" << current_entry.destAddr << "] nextAddr [" << current_entry.nextAddr << "]");
            break;
        }
      
      //------------------------------------------------------- END ADDED BY SERHANI --------------------------
        
        
                                            /*--------------- START DELETED BY SERHANI --------------
      bool added = false;
      const TopologySet &topology = m_state.GetTopologySet ();
      for (TopologySet::const_iterator it = topology.begin (); it != topology.end (); it++)
        {
      ...
      if (!added)
        break;
                                              --------------- END DELETED BY SERHANI --------------*/
    }
    
    SetRoutingTableEntriesToUnused();
      ...
      if (have_entry1 && !have_entry2)
        {
          ...
          AddEntry (tuple.ifaceAddr,
                    ...
                    entry1.interface,
                    entry1.distance,
                    entry1.metric,              //------- ADDED BY SERHANI ------------
                    true);                      //------- ADDED BY SERHANI ------------
        }
    }//End for 4.
  ...
  for (AssociationSet::const_iterator it = associationSet.begin ();
       it != associationSet.end (); it++)
    {
      ...
      else if(gatewayEntryExists && m_hnaRoutingTable->GetMetric (routeIndex) > gatewayEntry.metric)   //---- Modified By SERHANI from gatewayEntry.distance to gatewayEntry.metric
        {
      ...
      if(addRoute && gatewayEntryExists)
        {
          m_hnaRoutingTable->AddNetworkRouteTo (tuple.networkAddr,
                                                ...
                                                gatewayEntry.metric);   // ---- Modified by SERHANI from gatewayEntry.distance to gatewayEntry.metric
                               ...
                               const Ipv4Address &senderIface)
{
...
#ifdef NS3_LOG_ENABLE
  {
    ...
    //NS_LOG_DEBUG (Simulator::Now ().GetSeconds () << "s ** BEGIN dump Link Set for OLSR Node " << m_mainAddress);
    ...
    //NS_LOG_DEBUG (Simulator::Now ().GetSeconds () << "s ** BEGIN dump Neighbor Set for OLSR Node " << m_mainAddress);
  ...
  {
    const TwoHopNeighborSet &twoHopNeighbors = m_state.GetTwoHopNeighbors ();
    //NS_LOG_DEBUG (Simulator::Now ().GetSeconds () << "s ** BEGIN dump TwoHopNeighbor Set for OLSR Node " << m_mainAddress);
                            ...
                            const Ipv4Address &senderIface)
{
  ...
  //for (std::vector<Ipv4Address>::const_iterator i = tc.neighborAddresses.begin (); i != tc.neighborAddresses.end (); i++)  { //-------- DELETED By SERHANI ---------
        
  for(uint8_t i = 0; i < tc.neighborAddresses.size(); i++)
    {
      //const Ipv4Address &addr = *i;                                       //----------- COMMENT By SERHANI 12/12/2013 ---------
      const Ipv4Address &addr = tc.neighborAddresses.at(i);                 //---------- ADD By SERHANI 12/12/2013 --------
      ...
      if (topologyTuple != NULL)
        {
          ...
          topologyTuple->ETX = tc.GetETX(i);                           //----------- ADD SERHANI 12/12/2013 ---------------
        }
      else
        {
  ...
  {
    const TopologySet &topology = m_state.GetTopologySet ();
    //NS_LOG_DEBUG (Simulator::Now ().GetSeconds () << "s ** BEGIN dump TopologySet for OLSR Node " << m_mainAddress);
...
RoutingProtocol::QueueMessage (const olsr::MessageHeader &message, Time delay)
{
  ...
  if (not m_queuedMessagesTimer.IsRunning ())
    {
      //NS_LOG_DEBUG("in RoutingProtocol::QueueMessage SetDelay : " << delay);
                             ...
                             const MessageList &containedMessages)
{
  ...
  //NS_LOG_DEBUG ("[" << Simulator::Now ().GetSeconds () << "] OLSR node " << m_mainAddress << " sending a OLSR packet");
  ...
  for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator i =
         m_socketAddresses.begin (); i != m_socketAddresses.end (); i++)
    {
      ...
      //NS_LOG_UNCOND (Simulator::Now ().GetSeconds () << " OLSR Node [" << m_mainAddress << "] Sends Packet to [" << bcast << "]" );
...
RoutingProtocol::SendHello ()
{
  ...
  for (LinkSet::const_iterator link_tuple = links.begin ();
       link_tuple != links.end (); link_tuple++)
    {
      ...
      uint8_t link_type, nb_type = 0xff; // 0xff means the integer 255 which has the binary representation 00000000000000000000000011111111 (when using 32-bit integers)
      ...
      if (m_state.FindMprAddress (GetMainAddress (link_tuple->neighborIfaceAddr)))
        {
          ...
          //NS_LOG_DEBUG ("I consider neighbor " << GetMainAddress (link_tuple->neighborIfaceAddr) << " to be MPR_NEIGH.");
        }
      else
        {
          bool ok = false;
              ...
              if (nb_tuple->neighborMainAddr == GetMainAddress (link_tuple->neighborIfaceAddr))
                {
                  if (nb_tuple->status == NeighborTuple::STATUS_SYM)
                    {
                      //NS_LOG_DEBUG ("I consider neighbor " << GetMainAddress (link_tuple->neighborIfaceAddr) << " to be SYM_NEIGH.");
                      nb_type = OLSR_SYM_NEIGH;
                    }
                  else if (nb_tuple->status == NeighborTuple::STATUS_NOT_SYM)
                    {
                      nb_type = OLSR_NOT_NEIGH;
                      //NS_LOG_DEBUG ("I consider neighbor " << GetMainAddress (link_tuple->neighborIfaceAddr) << " to be NOT_NEIGH.");
                    }
                  else
                    {
      ...
      linkMessage.SetRLQ(link_tuple->RLQ); //---------- ADD SERHANI 11/12/2013 ------------
      linkMessage.SetETX(link_tuple->ETX); //---------- ADD SERHANI 11/12/2013 ------------
  ...
  //NS_LOG_DEBUG ("OLSR HELLO message size: " << int (msg.GetSerializedSize ()) << " (with " << int (linkMessages.size ()) << " link messages)");
...
RoutingProtocol::SendTc ()
{
  ...
  for (MprSelectorSet::const_iterator mprsel_tuple = m_state.GetMprSelectors ().begin ();
       mprsel_tuple != m_state.GetMprSelectors ().end (); mprsel_tuple++)
    {
      ...
      LinkTuple *link_tuple = m_state.FindLinkTuple (mprsel_tuple->mainAddr);    //------- ADD SERHANI 12/12/2013 ------------
      tc.SetETX (link_tuple->ETX);                                               //------- ADD SERHANI 12/12/2013 ------------
    }
                              ...
                              const Ipv4Address &senderIface)
{
  ...
  Time now = Simulator::Now ();
  uint16_t seqno = msg.GetMessageSequenceNumber ();                         //--------- ADD SERHANI 12/12/2013 --------
  ...
  if (link_tuple == NULL)
    {
      ...
      newLinkTuple.numHelloReceived = 1;                                    //--------- ADD SERHANI 12/12/2013 --------
      newLinkTuple.numHelloSupposed = 1;                                    //--------- ADD SERHANI 12/12/2013 --------
      newLinkTuple.lastPackSeqNum = seqno;                                  //--------- ADD SERHANI 12/12/2013 --------
      newLinkTuple.InitPackSeqNum = seqno;                                  //--------- ADD SERHANI 12/12/2013 --------
      newLinkTuple.RLQ = 1;                                                 //--------- ADD SERHANI 12/12/2013 --------
      newLinkTuple.DLQ = OLSR_UNDEFINED_LQ;                                 //--------- ADD SERHANI 12/12/2013 -------- We must verify if hello RLQ != OLSR_UNDEFINED_LQ
      newLinkTuple.ETX = OLSR_DEFAULT_METRIC;                               //--------- ADD SERHANI 12/12/2013 --------
        
        /*if (newLinkTuple.localIfaceAddr == "10.1.1.8") {
            NS_LOG_DEBUG("1st [" << Simulator::Now ().GetSeconds ()
                                 << "] From Node[" << senderIface
                                 << "] numHelloReceived[" << newLinkTuple.numHelloReceived
                                 << "] numHelloSupposed[" << newLinkTuple.numHelloSupposed
                                 << "] Seq No[" << seqno
                                 << "] lastPackSeqNum[" << seqno
                                 << "] InitPackSeqNum[" << seqno << "]");
        }*/
    ...
    {
      //------------------ START ADD SERHANI 12/12/2013 ------------------------
      //if (!(link_tuple->InitPackSeqNum == link_tuple->lastPackSeqNum && link_tuple->numHelloReceived = 0 &&   link_tuple->numHelloSupposed = 0))  //if we just begin new cycle
        /*{
            
            link_tuple->numHelloReceived = 1;
            link_tuple->numHelloSupposed = 1;
            
            
            if (link_tuple->localIfaceAddr == "10.1.1.8") {
                NS_LOG_DEBUG("1Cy [" << Simulator::Now ().GetSeconds ()
                             << "] From Node[" << senderIface
                             << "] numHelloReceived[" << link_tuple->numHelloReceived
                             << "] numHelloSupposed[" << link_tuple->numHelloSupposed
                             << "] Seq No[" << seqno
                             << "] lastPackSeqNum[" << link_tuple->lastPackSeqNum
                             << "] InitPackSeqNum[" << link_tuple->InitPackSeqNum << "]");
            }
        }
      else*/
        //{
            ...
            if (diff > OLSR_ETX_SEQNO_RESTART_DETECTION)
             {
                diff = 1;
             }
            ...
            /*if (link_tuple->localIfaceAddr == "10.1.1.8") {
                NS_LOG_DEBUG(" InC [" << Simulator::Now ().GetSeconds ()
                             << "] From Node[" << senderIface
                             << "] numHelloReceived[" << link_tuple->numHelloReceived
                             << "] numHelloSupposed[" << link_tuple->numHelloSupposed
                             << "] Seq No[" << seqno
          ...
          if (*neighIfaceAddr == receiverIface)
            {
              if (lt == OLSR_LOST_LINK)
              ...
              else if (lt == OLSR_SYM_LINK || lt == OLSR_ASYM_LINK)
                {
                  ...
                  link_tuple->DLQ = linkMessage->GetRLQ ();                   //------- ADDED BY SERHANI 12/12/2013 -----------
                    
                  //NS_LOG_DEBUG("GetRLQ : " << linkMessage->GetRLQ ());
            ...
            {
              //NS_LOG_DEBUG ("     \\-> *neighIfaceAddr (" << *neighIfaceAddr << " != receiverIface (" << receiverIface << ") => IGNORING!");
                
              //link_tuple->DLQ = OLSR_UNDEFINED_LQ;                            //------- ADDED BY SERHANI 12/12/2013 -----------
            }
        } //End 2nd For (neighborInterfaceAddresses)
                                                                                //NS_LOG_DEBUG ("Link tuple updated: " << int (updated));
    }// End 1st For (link messages)
  ...
  if (link_tuple->DLQ == OLSR_UNDEFINED_LQ)
    {
        link_tuple->ETX = OLSR_DEFAULT_METRIC;
          if (link_tuple->localIfaceAddr == "10.1.1.25") {
              //NS_LOG_DEBUG(Simulator::Now ().GetSeconds () << " ETX[" << link_tuple->ETX << "] [" << receiverIface << ", " << senderIface << "]");
          }
    }
  else
    {
        link_tuple->ETX = link_tuple->RLQ * link_tuple->DLQ;
          ...
          if (link_tuple->localIfaceAddr == "10.1.1.25") {
              //NS_LOG_DEBUG(Simulator::Now ().GetSeconds () << " RLQ[" << link_tuple->RLQ << "]");
              //NS_LOG_DEBUG(Simulator::Now ().GetSeconds () << " DLQ[" << link_tuple->DLQ << "]");
              //NS_LOG_DEBUG(Simulator::Now ().GetSeconds () << " ETX[" << link_tuple->ETX << "] [" << receiverIface << ", " << senderIface << "]");
          }
    }
  ...
  if (created)
    {
      ...
      m_events.Track (Simulator::Schedule (OLSR_ETX_UPDATE_TIME,                 //------- ADD SERHANI 12/12/2013 ----------
                           &RoutingProtocol::EtxTimerExpire, this,
                           link_tuple->neighborIfaceAddr));
    }
  //NS_LOG_DEBUG ("@" << now.GetSeconds () << ": Olsr node " << m_mainAddress << ": LinkSensing END");
}
       ...
       link_tuple != m_state.GetLinks ().end (); link_tuple++)
    {
      ...
      for (LinkMessageVec::const_iterator linkMessage = hello.linkMessages.begin ();
           linkMessage != hello.linkMessages.end (); linkMessage++)
        {
          ...
          //const char *neighborTypeName = ((neighborType < 3) ? neighborTypeNames[neighborType] : "(invalid value)");
          //NS_LOG_DEBUG ("Looking at Link Message from HELLO message: neighborType=" << neighborType << " (" << neighborTypeName << ")");
#endif //NS3_LOG_ENABLE
              ...
              if (neighborType == OLSR_SYM_NEIGH || neighborType == OLSR_MPR_NEIGH)
                {
                  ...
                  //NS_LOG_LOGIC ("Adding the 2-hop neighbor" << (nb2hop_tuple ? " (refreshing existing entry)" : ""));
                  if (nb2hop_tuple == NULL)
                    {
                      ...
                      new_nb2hop_tuple.ETX = linkMessage->GetETX();       //-------------- ADD SERHANI 12/12/2013 ----------------
                                                                          // we will add (link_tuple->ETX) to new_nb2hop_tuple.ETX (40% sure)
                  ...
                  else
                    {
                      ...
                      nb2hop_tuple->ETX = linkMessage->GetETX();           //-------------- ADD SERHANI 12/12/2013 ----------------
                    }
                }
  ...
  if (nb_tuple != NULL)
    {
      ...
      if (hasSymmetricLink)
        {
          nb_tuple->status = NeighborTuple::STATUS_SYM;
          //NS_LOG_DEBUG (*nb_tuple << "->status = STATUS_SYM; changed:" << int (statusBefore != nb_tuple->status));
        }
      else
        {
          nb_tuple->status = NeighborTuple::STATUS_NOT_SYM;
          //NS_LOG_DEBUG (*nb_tuple << "->status = STATUS_NOT_SYM; changed:" << int (statusBefore != nb_tuple->status));
        }
    }
  else
    {
...
RoutingProtocol::AddTopologyTuple (const TopologyTuple &tuple)
{
  //NS_LOG_FUNCTION (this);
   ...
   /*debug("%f: Node %d adds topology tuple: dest_addr = %d last_addr = %d seq = %d\n",
         ...
         tuple->seq());*/
    //NS_LOG_UNCOND("Add TopologyTuple");
...
//--------------------------------------------------------------------------------------------------- EtxTimerExpire ADDED BY SERHANI ------------------------------------------------------------   
//----------- START ADD SERHANI -------------------------
void
RoutingProtocol::EtxTimerExpire (Ipv4Address neighborIfaceAddr)
{
   ...
   if (tuple == NULL)
    ...
    /*if (tuple->localIfaceAddr == "10.1.1.8") {
        NS_LOG_DEBUG(Simulator::Now ().GetSeconds ()
                     << "--- EtxTimerExpire --- FromNode[" << neighborIfaceAddr
                     << "] lastPackSeqNum[" << tuple->lastPackSeqNum
                     << "] InitPackSeqNum[" << tuple->InitPackSeqNum
                     << "] numHelloReceived[" << tuple->numHelloReceived
                     << "] numHelloSupposed[" << tuple->numHelloSupposed << "]");
    }*/
    
   m_events.Track (Simulator::Schedule (OLSR_ETX_UPDATE_TIME,
                        &RoutingProtocol::EtxTimerExpire, this,
                        neighborIfaceAddr));
}
//----------- END ADD SERHANI -------------------------
...
{
    //ADDED BY SERHANI -------------------------------------------------------------------------------------------------
    /*if (m_mainAddress =="10.1.1.9")
    {
        NS_LOG_UNCOND("----------------------------- [" << Simulator::Now ().GetSeconds () << "] [" <<  m_mainAddress << "] RouteOutput ------------------------------");
        Dump();
        NS_LOG_UNCOND("----------------------- Print Header -------------------");
  ...
  if (Lookup (header.GetDestination (), entry1) != 0)
    {
      ...
      /*if (m_mainAddress =="10.1.1.9")
      {
          NS_LOG_UNCOND("-> Entry [" << entry1.destAddr << ", "<< entry1.nextAddr << ", " << entry1.interface << ", " << entry1.distance << ", " <<  entry1.metric << "]" );
      }*/
      ...
      /*if (m_mainAddress =="10.1.1.9")
        {
            NS_LOG_UNCOND("-> Send Entry [" << entry2.destAddr << ", "<< entry2.nextAddr << ", " << entry2.interface << ", " << entry2.distance << ", " <<  entry2.metric << "]" );
        }*/
  ...
  if (!found)
    {
      ...
      /*if (m_mainAddress =="10.1.1.9")
        {
            NS_LOG_UNCOND("-> No Route to host: sockerr [" << sockerr << "]" );
        }*/
    }
  return rtentry;
}
...
{
    //ADDED BY SERHANI -------------------------------------------------------------------------------------------------
    if ( Simulator::Now ().GetSeconds () >=40.3 &&  Simulator::Now ().GetSeconds () <= 40.7 && m_mainAddress =="10.1.1.9")
    {
        /*NS_LOG_UNCOND("----------------------------- [" << Simulator::Now ().GetSeconds () << "] [" <<  m_mainAddress << "] RouteInput ------------------------------");
        Dump();
        NS_LOG_UNCOND("----------------------- Print Header -------------------");
        std::ostringstream oss;
        header.Print(oss);
        std::cout << oss.str() << '\n';
        NS_LOG_UNCOND("-> Destination [" << header.GetDestination () << "]");*/
    }
  
  //NS_LOG_UNCOND("[" << Simulator::Now ().GetSeconds () << "] RouteInput");
  ...
  if (Lookup (header.GetDestination (), entry1))
    ...
    //ADDED BY SERHANI -------------------------------------------------------------------------------------------------
    if ( Simulator::Now ().GetSeconds () >=40.3 &&  Simulator::Now ().GetSeconds () <= 40.7 && m_mainAddress =="10.1.1.15")
        {
            /*NS_LOG_DEBUG ("Olsr node " << m_mainAddress
...
RoutingProtocol::AddEntry (Ipv4Address const &dest,
                           ...
                           uint32_t interface,
                           uint32_t distance,
                           uint32_t metric,         // --- ADDED By SERHANI ---
                           bool used)               // --- ADDED By SERHANI ---
{
  //NS_LOG_FUNCTION (this << dest << next << interface << distance << m_mainAddress);

  NS_ASSERT (distance > 0 && metric > 0); // --- MODIFIED By SERHANI ---
  ...
  entry.metric = metric;     // --- ADDED By SERHANI ---
  entry.used = used;         // --- ADDED By SERHANI ---
}
      ...
      for (uint32_t j = 0; j < m_ipv4->GetNAddresses (i); j++)
        {
          if (m_ipv4->GetAddress (i,j).GetLocal () == interfaceAddress)
            {
              AddEntry (dest, next, i, distance, metric, used);   // --- MODIFIED By SERHANI ---
              return;
            }
        }
    }
  NS_ASSERT (false); // should not be reached
  AddEntry (dest, next, 0, distance, metric, used);         // --- MODIFIED By SERHANI ---
}

//------------------------------------------------------------------------------------------------------------- GetRoutingTableEntries -------------------------------------------------------
std::vector<RoutingTableEntry>
RoutingProtocol::GetRoutingTableEntries ()
{
  //NS_LOG_FUNCTION (this);
    
  std::vector<RoutingTableEntry> retval;
    
  for (std::map<Ipv4Address, RoutingTableEntry>::iterator iter = m_table.begin ();
       iter != m_table.end (); iter++)
    {
